...
function App(){
  ...
  const [nameSpace, setNameSpace] = useState('Mi espacio de trabajo')
  const [color, setColor] = useState('black')
  const [numPers, setNumPers] = useState('0')
  const [URLSpace, setURLSpace] = useState('https://mi.dominio.dofleini.com')
  const [messageApi, contextHolder] = message.useMessage();
  const changeNameSpace = (e) => {
    setNameSpace(e.target.value)
  }
  const changeURLSpace = (e) => {
    setURLSpace('https://'+e.target.value+'.dofleini.com')
  }
  const changeColor = (e) => {
    setColor(e.target.value)
  }
  const changeNumPers = (e) => {
    setNumPers(e.target.value)
  }
  const uploadLogo = (file) => {
    //this.setState({logo: 'ok'})
  ...
  const submit = (e) => {
        fetch('/submit/',{
          ...
          if(response.status===201){
            ...
            messageApi.warning('Error de estado '+response.status.toString())
          }
        }).catch((e)=>{
          messageApi.error('Error!')
        }).finally(()=>{
          console.info('Fin de la request!')
        })
      }
  return (
    <>
